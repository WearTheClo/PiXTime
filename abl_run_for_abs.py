...
import models.PiXTime_No_abs_token as PiXTime
...
def work_process(args):
    ...
    model = PiXTime.Model(seq_len = args.seq_len, pred_len = args.pred_len, patch_len = args.patch_len, n_vars = enc_in, d_model = args.d_model,
                          ...
                          en_layers = args.en_layers, de_layers = args.de_layers, features = args.features).to(args.device)
...
def run():
    ...
    parser.add_argument('--model', type=str, required=False, default='PiXTime',
                        help='model name, options: [PiXTime]')
    parser.add_argument('--evaluation', type=str, default='./evaluation/NoAbs')
