...
from layers.Embed import DataEmbedding_inverted, PositionalEmbedding, LearnablePE, PositionalEmbedding
import numpy as np
...
class Patch_EnEmbedding(nn.Module):
    ...
    #输入(B, n_target_vars, seq_len)
        ...
        x = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3])) #[b_s * n_target_vars, patch_num + 1, patch_len]
...
class iTrans_ExEmbedding(nn.Module):
    def __init__(self, 
        ...
        self.var_embedding = LearnablePE(d_model = d_model, max_len = max_aux_var_num) #可学习VE
        #self.var_embedding = PositionalEmbedding(d_model = d_model, max_len = max_aux_var_num) #静态VE，用于VE消融
    ...
    def forward(self, x_enc):
        ...
        enc_out = enc_out + self.var_embedding(enc_out) #可学习VE与静态VE效果差不多，不用VE差的有点多
...
class XDecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, tau=None, delta=None):
        ...
        x = torch.cat([x[:, :-1, :], x_glb], dim=1) #拼回x
