...
def work_process(args):
    ...
    if args.model == 'DLinear':
        ...
        model = TimeFilter.Model(seq_len = args.seq_len, pred_len = args.pred_len, patch_len = args.patch_len, enc_in = enc_in, d_model = args.d_model,
                                 dropout = args.dropout, n_heads = args.n_heads, d_ff = args.en_d_ff, e_layers = args.en_layers).to(args.device)
    ...
    if args.model == 'PiXTime':
        ...
        checkpoint_path = checkpoint_dir + '/' + file_name + '.pt'
...
def run():
    ...
    if args.seq_len % args.patch_len != 0 :
        raise ValueError("patch_len error")
    file_name  = args.model + args.data + 'PatL' + str(args.patch_len) + 'PreL' + str(args.pred_len) + 'Fea' + args.features + 'DM' + str(args.d_model) + 'ED' + str(args.en_d_ff) + 'DD' + str(args.de_d_ff) + 'EL' + str(args.en_layers) + 'DL' + str(args.de_layers)
